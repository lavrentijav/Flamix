...
6. Скачайте пакет клиента и сохраните одноразовый пароль из HTTP-заголовка `X-Flamix-Client-Key-Password`
7. Перед запуском клиента задайте этот пароль в `FLAMIX_CLIENT_KEY_PASSWORD`
...
- **SQLite storage**: файл БД не шифруется целиком; сервер защищает только выбранные секреты на уровне приложения
- **TLS соединения**: Mutual TLS для клиент-сервер связи с привязкой `client_id` к identity клиентского сертификата
- **Diffie-Hellman**: обмен эфемерными ключами; активные session keys держатся в памяти и не пишутся в SQLite
- **HMAC**: Проверка целостности сообщений
- **Nonce и timestamps**: Защита от replay-атак

## База данных

Проект использует обычный SQLite. Полноценное at-rest шифрование всей БД не реализовано; сервер защищает только выбранные секреты, например старые ключи ротации. Для тестов можно отключить эту защиту:
